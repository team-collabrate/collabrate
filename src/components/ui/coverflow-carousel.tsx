...
export function CoverflowCarousel({
  ...
  /** True once a pointer gesture has moved far enough to count as a drag
      rather than a click/tap — lets a card's onClick ignore drag-releases. */
  ...
  const onPointerMove = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    // A few pixels of wobble on a tap shouldn't cancel the click that
    // follows — only count it as a drag once it's travelled a real amount.
  ...
  const onCardClick = (index: number) => {
    // A drag that just ended fires a click on release — ignore it so a
    // swipe never doubles as an accidental jump-to-card.
          ...
          >
            {slides.map((slide, index) => (
              <div
                ...
                className={cn(
                  "absolute left-1/2 top-0 aspect-square overflow-hidden rounded-2xl bg-muted shadow-xl transition-[filter] duration-200 will-change-transform",
                  index === selected ? "cursor-default" : "cursor-pointer hover:brightness-110",
                  cardClassName,
                )}
